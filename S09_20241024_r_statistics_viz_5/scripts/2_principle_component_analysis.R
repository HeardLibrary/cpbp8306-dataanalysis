...
####################################################
pca_input <- subset(all_data, select = c(vulnerability_2020:open_out_groups))
pca_output <- PCA(pca_input, scale.unit = TRUE, ncp = 6, graph = FALSE)
#pca_var_rotated <- varimax(pca_output$var$coord)
#pca_ind_rotated <- varimax(pca_output$ind$coord)
#pca_output$var$coord <- pca_var_rotated$loadings
#pca_output$ind$coord <- pca_ind_rotated$loadings
...
#Individual biplots
pca_ind_biplot_1_2 <- fviz_pca_ind(pca_output, geom = "text", col.ind = "cos2",
                                   gradient.cols = c("black", "orange", "green"),
                                   repel = TRUE)
pca_ind_biplot_1_2
...
# Barplots of individual contributions
pca_dim1_contrib_ind <- fviz_contrib(pca_output, choice="ind", axes = 1, top=30)
pca_dim1_contrib_ind
pca_dim2_contrib_ind <- fviz_contrib(pca_output, choice="ind", axes = 2, top=30)
pca_dim2_contrib_ind

# Making the plots yourself
contributions_country <- cbind(all_data$iso_code, pca_output$ind$contrib)
contributions_country <- data.frame(contributions_country)
contributions_country$Dim.1 <- as.numeric(contributions_country$Dim.1)
contributions_country$Dim.2 <- as.numeric(contributions_country$Dim.2)
contributions_country$V1 <- reorder(contributions_country$V1, contributions_country$Dim.1, max)
contributions_country %>% arrange(desc(Dim.1)) %>% head(30) %>%
  ggplot(aes(x=V1, y=Dim.1)) +
  geom_bar(stat="identity", fill="blue") +
  scale_y_continuous(breaks = seq(1, 5, 1)) +
  labs(title="Individual contributions to Dimension 1", x="Country", y="Contribution") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
contributions_country$V1 <- reorder(contributions_country$V1, contributions_country$Dim.2, max)
contributions_country %>% arrange(desc(Dim.2)) %>% head(30) %>%
  ggplot(aes(x=V1, y=Dim.2)) +
  geom_bar(stat="identity", fill="blue") +
  scale_y_continuous(breaks = seq(1, 5, 1)) +
  labs(title="Individual contributions to Dimension 2", x="Country", y="Contribution") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
co2_dim2_plot <- ggplot(all_data_plot, aes(x=dim_2, y=co2_per_capita, color=wvs_region, label = iso_code)) +
  ...
  scale_x_continuous(breaks = -3:5) +
  scale_y_continuous() +
  labs(x="Dimension 2", y="CO2 per capita") + 
  theme_classic() + co2_theme + theme(legend.position = "none") +
