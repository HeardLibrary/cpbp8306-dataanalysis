...
# Save to variables
mean_value <- mean(na.omit(schools_data$Grade.1))
median_value <- median(na.omit(schools_data$Grade.1))
sd_value <- sd(na.omit(schools_data$Grade.1))
var_value <- var(na.omit(schools_data$Grade.1))
min_value <- min(na.omit(schools_data$Grade.1))
max_value <- max(na.omit(schools_data$Grade.1))
sum_value <- sum(na.omit(schools_data$Grade.1))

# Create the table
statistics_table <- data.frame(
  Statistic = c("Mean", "Median", "Standard Deviation", "Variance", "Minimum", "Maximum", "Sum"),
  Value = round(c(mean_value, median_value, sd_value, var_value, min_value, max_value, sum_value), 1)
)
